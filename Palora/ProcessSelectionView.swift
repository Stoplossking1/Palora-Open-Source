...
struct ProcessSelectionView: View {
    ...
    private func createRecorder() {
        ...
        do {
            let session = try recordingFileManager.prepareSession(appName: tap.process.name, startedAt: Date())
            let newRecorder = ProcessTapRecorder(fileURL: session.audioURL, tap: tap)
